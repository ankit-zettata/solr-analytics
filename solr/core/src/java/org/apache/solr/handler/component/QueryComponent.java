...
public class QueryComponent extends SearchComponent
{
  ...
  private void createMainQuery(ResponseBuilder rb) {
    ...
    // distributed search doesn't REALLY require this if all the user is interested in
    // is facet, stats, and things that can be aggregated as a whole [facets for example]
    String uniqueId = rb.req.getSchema().getUniqueKeyField() != null ?rb.req.getSchema().getUniqueKeyField().getName() : ""; 
    
    if (false == StringUtils.isEmpty(uniqueId)) {
      if ( (rb.getFieldFlags() & SolrIndexSearcher.GET_SCORES)!=0 || rb.getSortSpec().includesScore()) {
        sreq.params.set(CommonParams.FL, uniqueId + ",score");
      } else {
        sreq.params.set(CommonParams.FL, uniqueId);      
      }
    }

...
public class SolrRequestParsers 
{
  ...
  public SolrQueryRequest buildRequestFrom( SolrCore core, SolrParams params, Collection<ContentStream> streams ) throws Exception
  {
    ...
    // Handle VFS files (allows CSV and other content to be gzip and come from alternate sources, like WebDAV, SSH, etc)
    strs = params.getParams( CommonParams.STREAM_VFS );
    if( strs != null ) {
      if( !enableRemoteStreams ) {
        throw new SolrException( SolrException.ErrorCode.BAD_REQUEST, "Remote Streaming is disabled." );
      }
      for( final String vfsUri : strs ) {
        ContentStreamBase stream = new ContentStreamBase.VFSStream(vfsUri);
        if( contentType != null ) {
          stream.setContentType( contentType );
        }
        streams.add( stream );

...
public class SimpleFacets {
  ...
  public NamedList<Object> getFacetCounts() {
    ...
    if (!params.getBool(FacetParams.FACET,true) && !params.getBool(FacetParams.PERCENTILE, true)) {
      return null;
    }
    ...
    try {
      ...
      if(params.getBool(FacetParams.PERCENTILE, false)) {
    	  facetResponse.add("facet_statistics", getFacetPercentileCounts());
      }
...
/**
  ...
  private RangeEndpointCalculator<?> getCalc(SchemaField sf, FieldType ft) {
  ...
  private <T extends Comparable<T>> NamedList getFacetRangeCounts
    ...
    T low = start;
    RangeCounter rangeCounter = new RangeCounter(calc, start, end, gap, low, searcher, base, sf);
    rangeCounter.includeEdge = include.contains(FacetRangeInclude.EDGE);
    rangeCounter.includeLower = include.contains(FacetRangeInclude.LOWER);
    rangeCounter.includeUpper = include.contains(FacetRangeInclude.UPPER);
    rangeCounter.rangeHardEnd = params.getBool(FacetParams.FACET_RANGE_HARD_END, false);
    while (rangeCounter.hasMoreCounts()) {
      CountPair<String, Integer> currentCount = rangeCounter.getNextCount();
      final int count = currentCount.val;
      if (count >= minCount) {
        counts.add(currentCount.key, count);
      }
    }
    end = (T)rangeCounter.getEnd();
   ...
   * and private.
   */
  static abstract class RangeEndpointCalculator<T extends Comparable<T>> {
    ...
    public final T findMiddle(T value, String gap) {
    	try {
    		return parseAndFindMiddle(value, gap);    		
    	} catch (Exception e) {
    		throw new SolrException(SolrException.ErrorCode.BAD_REQUEST,
    								"Can't find middle of "+value+" plus "+gap+
    								" for field: " + field.getName(), e);
    	}
    }
    
    protected abstract T parseAndFindMiddle(T value, String gap)
    	throws java.text.ParseException;
  ...
  private static class FloatRangeEndpointCalculator 
	...
	@Override
	public Float parseAndFindMiddle(Float value, String gap)
			throws java.text.ParseException {
		return new Float(value.floatValue() + (Float.valueOf(gap).floatValue() / 2));
	}
  }
  private static class DoubleRangeEndpointCalculator 
	...
	@Override
	public Double parseAndFindMiddle(Double value, String gap)
			throws java.text.ParseException {
		return new Double(value.doubleValue() + (Double.valueOf(gap).doubleValue() / 2));
	}
  }
  private static class IntegerRangeEndpointCalculator 
	...
	@Override
	public Integer parseAndFindMiddle(Integer value, String gap)
			throws java.text.ParseException {
		return new Integer(value.intValue() + (Integer.valueOf(gap).intValue() / 2));
	}
  }
  private static class LongRangeEndpointCalculator 
	...
	@Override
	public Long parseAndFindMiddle(Long value, String gap)
			throws java.text.ParseException {
		return new Long(value.longValue() + (Long.valueOf(gap).longValue() / 2));
	}
  }
  private static class DateRangeEndpointCalculator 
	...
	@Override
	public Date parseAndFindMiddle(Date value, String gap)
			throws java.text.ParseException {
		Calendar startCal = Calendar.getInstance(DateField.UTC, Locale.US);
		Calendar endCal = Calendar.getInstance(DateField.UTC, Locale.US);		
		Date endDate = this.parseAndAddGap(value, gap);
		Date startDate = value;
		startCal.setTime(startDate);
		endCal.setTime(endDate);
		long differenceInMillis = endCal.getTimeInMillis() - startCal.getTimeInMillis();
		Date middleDate = new Date(startCal.getTimeInMillis() + (differenceInMillis / 2));
		return middleDate;		
	}
  }
